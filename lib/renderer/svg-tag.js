...
const komojuColor1 = '#172E44'
const komojuColor2 = '#1880DE'
...
function qrToShapes (qr, opts) {
  const size = qr.modules.size
  const data = qr.modules.data
  const margin = opts.margin
  const palette = opts.color

  let result = ''

  for (let i = 0; i < size; i++) {
    for (let j = 0; j < size; j++) {
      const color = palette(data[i * size + j], i, j, size)
      const x = margin+j;
      const y = margin+i;
...
exports.render = function render (qrData, options, cb) {
  ...
  const crap = { light: opts.color(false), dark: opts.color(true) }

  const bg = !crap.light.a
    ? ''
    : '<path ' + getColorAttrib(crap.light, 'fill') +
      ' d="M0 0h' + qrcodesize + 'v' + qrcodesize + 'H0z"/>'

  const path =
    '<path ' + getColorAttrib(crap.dark, 'stroke') +
    ' d="' + qrToPath(data, size, opts.margin) + '"/>'
  //const shapes = qrToShapes(qrData, opts)
  ...
  const svgTag =
    ...
    '</svg>\n'
  //const svgTag = '<svg xmlns="http://www.w3.org/2000/svg" style="--size: ' + size + '" ' + width + viewBox + ' shape-rendering="crispEdges">' + bg + shapes + '</svg>\n'
